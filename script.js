    ...
    btn.addEventListener("click", (e) => {
      ...
      modalContent.innerHTML = `
        <div class="blog-modal-body">
          ...
          <div class="blog-modal-meta"><time datetime="\( {post.dateISO}"> \){post.date}</time> • ${post.readTime}</div>
          ${post.content}
      ...
      modalContent.querySelectorAll(".blog-toc a").forEach(link => {
        link.addEventListener("click", (ev) => {
          ev.preventDefault();
          const t = modalContent.querySelector(link.getAttribute("href"));
...
document.addEventListener("DOMContentLoaded", function() {
  if (typeof initBlogArticleModal === "function") initBlogArticleModal();
});
